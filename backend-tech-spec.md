...
## 8. Структура темы
- Корневая директория содержит базовые шаблоны WordPress (404.php, index.php, archive-*.php, single-*.php, page-*.php) и вспомогательные файлы (`functions.php`, `header.php`, `footer.php`, `style.css`, `flush_rewrite.php`). Эта зона отвечает за точку входа шаблонов и нельзя удалять существующие файлы без миграционного плана.
- Каталог `inc/` — бизнес-логика темы: регистрация CPT/таксономий, меню, breadcrumbs, SEO, AJAX-формы и Carbon Fields. Внутри лежит вложенный пакет `inc/carbon-fields/` (composer-зависимости, vendor). Новые PHP-модули добавлять именно сюда и подключать из `functions.php`.
- Каталог `assets/` — сбилженные и исходные фронтенд-ресурсы. Подкаталоги `assets/js` (основной код: main.js, common.js, мобильное меню и пр.), `assets/css` (main.css, style.css, Gutenberg стили), `assets/fonts`, `assets/images` с подразделами promo/stock/team и т.д. Для новых бандлов придерживаться этой структуры и прописывать связь в `inc/assets.php`.
- Каталог `templates/` хранит кастомные page templates (home.php, prices.php). `templates-parts/` содержит частичные блоки (sidebar.php, widgets-modals.php, paginations.php, contact-form.php). Соглашение: любая разметка, используемая в нескольких местах, выносится в templates-parts.
- Каталог `assets/build/` зарезервирован под итоговые бандлы JS (common.js) и не редактируется вручную — только через инструмент сборки.
- Дополнительные ресурсы: `docs/` (текущая документация), `inc.txt` (служебная памятка), шрифты и медиа. При разработке новых функций обновляйте документацию и поддерживайте единый стиль именования файлов.